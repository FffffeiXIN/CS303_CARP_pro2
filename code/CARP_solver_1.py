...
if __name__ == '__main__':
    ...
    for rule in range(5):
        ...
        if cur_total_cost < OPT_cost:
            OPT_cost = cur_total_cost
            OPT_route = cur_route
            OPT_load = cur_load
    # print_res()

    for i in range(50000):
        OPT_route, OPT_cost = swap()
    OPT_route, OPT_cost = flipping()
    for i in range(50000):
        OPT_route, OPT_cost = cross_double_insertion()
    OPT_route, OPT_cost = flipping()
    for i in range(50000):
        OPT_route, OPT_cost = cross_single_insertion()
    OPT_route, OPT_cost = flipping()
    for i in range(50000):
        OPT_route, OPT_cost = self_insertion()
    OPT_route, OPT_cost = flipping()
    # 288	345	5862	3978	323	428	188
