...
def single_2_opt(OPT_route, OPT_cost, OPT_load):
    ...
    if old_cost > new_cost:
        ...
        return routes, OPT_cost, OPT_load
    return OPT_route, OPT_cost, OPT_load
...
def cross_2_opt(OPT_route, OPT_cost, OPT_load):
    ...
    if child_cost2 <= child_cost1 and child_cost2 < OPT_cost:
        ...
        OPT_load[r1_index] = demand1 + demand3
        OPT_load[r2_index] = demand2 + demand4
        routes[r1_index] = new_route1
        routes[r2_index] = new_route2
        # print("进来过2")
        return routes, OPT_cost, OPT_load
...
if __name__ == '__main__':
    # input_file = 'E:\大三上\AI\project2_carp\CARP\CARP_samples\gdb1.dat'
    ...
    best_route = []
    best_load = []
    weight = [0.05, 0.15, 0.30, 0.50, 0.7, 0.85]
    for rule in range(5):
        ...
        for i in range(100000):
            ...
            if r < weight[0]:
                ...
                cur_route, cur_cost, cur_load = single_2_opt(cur_route, cur_cost, cur_load)
            else:
                cur_route, cur_cost, cur_load = cross_2_opt(cur_route, cur_cost, cur_load)
        cur_route, cur_cost = flipping(cur_route, cur_cost)
        if cur_cost < best_cost:
            best_route = cur_route
            best_cost = cur_cost

    # 288	329	5482	3808	311	428	186

    # for rule in range(5):
    #     cur_route, cur_load, cur_total_cost = path_scanning(rule)
    #     if cur_total_cost < best_cost:
    #         best_cost = cur_total_cost
    #         best_route = cur_route
    #         best_load = cur_load
    # for i in range(50000):
    #     best_route, best_cost, best_load = cross_2_opt(best_route, best_cost, best_load)
    # best_route, best_cost = flipping(best_route, best_cost)
